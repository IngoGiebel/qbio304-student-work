...
abundance_filepaths_onivara <- file.path(
  studydesign_onivara_df$`Run accession`,
  "abundance.tsv")

# - of the Oryza sativa samples
abundance_filepaths_osativa <- file.path(
  studydesign_osativa_df$`Run accession`,
  "abundance.tsv")

# --- Get annotations using BiomaRt

# BioMaRt dataset for Oryza nivara
biomart_onivara <- biomaRt::useMart(
  biomart = "plants_mart",
...
biomart_osativa <- biomaRt::useMart(
  biomart = "plants_mart",
...
txi_gene_onivara <- tximport::tximport(
  abundance_filepaths_onivara,
  type = "kallisto",
  tx2gene = tx2gene_onivara,
  countsFromAbundance = "lengthScaledTPM")

txi_gene_osativa <- tximport::tximport(
  abundance_filepaths_osativa,
  type = "kallisto",
  tx2gene = tx2gene_osativa,
  countsFromAbundance = "lengthScaledTPM")
